...
public class JCVideoPlayer extends FrameLayout implements View.OnClickListener, SeekBar.OnSeekBarChangeListener, SurfaceHolder.Callback, View.OnTouchListener, JCMediaManager.JCMediaPlayerListener {
    ...
    LinearLayout llVotocastLikeCountTestControls, llVotocastPlayCountTestControls, llVotocastShareCountTestControls;
    TextView tvListItemVideoLikeTestControls, tvListItemVideoPlayTestControls, tvListItemVideoShareTestControls;
    ImageView ivHomeVideoFullScreenTestControls, ivHomeVideoMuteTestControls;
    int isMute = 0, views;
    int videoId;
    String token, video_url="",Authorization;
    ...
    public static int CURRENT_STATE = -1;//-1相当于null
    ...
    private void init(Context context) {
        ...
        llVotocastLikeCountTestControls = (LinearLayout) findViewById(R.id.llVotocastLikeCountTestControls);
        llVotocastPlayCountTestControls = (LinearLayout) findViewById(R.id.llVotocastPlayCountTestControls);
        llVotocastShareCountTestControls = (LinearLayout) findViewById(R.id.llVotocastShareCountTestControls);

        tvListItemVideoLikeTestControls = (TextView) findViewById(R.id.tvListItemVideoLikeTestControls);
        tvListItemVideoPlayTestControls = (TextView) findViewById(R.id.tvListItemVideoPlayTestControls);
        tvListItemVideoShareTestControls = (TextView) findViewById(R.id.tvListItemVideoShareTestControls);

        ivHomeVideoFullScreenTestControls = (ImageView) findViewById(R.id.ivHomeVideoFullScreenTestControls);
        ivHomeVideoMuteTestControls = (ImageView) findViewById(R.id.ivHomeVideoMuteTestControls);

        ivHomeVideoFullScreenTestControls.setOnClickListener(this);
        ivHomeVideoMuteTestControls.setOnClickListener(this);
     ...
     * @param title | title
     */
    public void setVideoLike(final String like) {
//        Log.i("VideoView","Like - " + like);
        tvListItemVideoLikeTestControls.setText(like);
    }
    public void setVideoViews(String view) {
//        Log.i("VideoView", "view- " + view);
        tvListItemVideoPlayTestControls.setText(view);
        views = Integer.parseInt(view);
    }
//
//    public void setVideoShare(String share) {
//        Log.i("VideoView","share- " + share);
//        tvListItemVideoShareTestControls.setText(share);
//    }

    public void setVideoId(int videoId, String token, String video_url,String Authorization) {
     ...
     * @param url         | Video address
     * @param title       | title
     * @param ifShowTitle | The title is displayed in full-screen under
     */
    public void setUp(String url, String title, boolean ifShowTitle) {
        ...
        if (ifFullScreen) {
            ...
            ivHomeVideoFullScreenTestControls.setImageResource(enlargRecId == 0 ? R.drawable.shrink_video : enlargRecId);
            llVotocastLikeCountTestControls.setVisibility(View.GONE);
            llVotocastPlayCountTestControls.setVisibility(View.GONE);
            llVotocastShareCountTestControls.setVisibility(View.GONE);

        } else {
            ivFullScreen.setImageResource(shrinkRecId == 0 ? R.drawable.enlarge_video : shrinkRecId);
            ivHomeVideoFullScreenTestControls.setImageResource(shrinkRecId == 0 ? R.drawable.enlarge_video : shrinkRecId);
...
//        Log.i("FullScreen set",url + "-" + title);
        ...
        CURRENT_STATE = CURRENT_STATE_NORMAL;
        if (ifFullScreen) {
            llVotocastPlayCountTestControls.setVisibility(View.INVISIBLE);
            ivFullScreen.setImageResource(shrinkRecId == 0 ? R.drawable.shrink_video : shrinkRecId);
            ivHomeVideoFullScreenTestControls.setImageResource(shrinkRecId == 0 ? R.drawable.shrink_video : shrinkRecId);
        } else {
            llVotocastPlayCountTestControls.setVisibility(View.VISIBLE);
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.start || i == R.id.thumb) {
            if (TextUtils.isEmpty(url)) {
                Toast.makeText(getContext(), "Video address is null", Toast.LENGTH_SHORT).show();
                return;
            }
            if (i == R.id.thumb) {
            ...
            if (CURRENT_STATE == CURRENT_STATE_NORMAL || CURRENT_STATE == CURRENT_STATE_ERROR) {
                ...
                if(!video_url.equals("") && video_url != null)
                    new setViewVideo().execute();

                tvListItemVideoPlayTestControls.setText(String.valueOf(views + 1));
            ...
            quitFullScreen();
        } else if (i == R.id.ivHomeVideoMuteTestControls) {
            if (isMute == 0) {
                isMute = 1;
                JCMediaManager.intance().mediaPlayer.setVolume(0, 0);
                ivHomeVideoMuteTestControls.setImageResource(R.drawable.mute);
            } else {
                isMute = 0;
                JCMediaManager.intance().mediaPlayer.setVolume(1, 1);
                ivHomeVideoMuteTestControls.setImageResource(R.drawable.sound);
            }
        } else if (i == R.id.ivHomeVideoFullScreenTestControls) {
...
//                Log.i("Volume","R.id.ivHomeVideoFullScreenTestControls - "+isMute);
                if(isMute==1)
                    JCMediaManager.intance().mediaPlayer.setVolume(0, 0);
                else
                    JCMediaManager.intance().mediaPlayer.setVolume(1, 1);
...
//        Log.i("Volume","stopToFullscreenOrQuitFullscreenShowDisplay - "+isMute);
...
//        Log.i("Volume","surfaceCreated - "+isMute);
...
//            JCMediaManager.intance().mediaPlayer.stop();
    ...
    public static void setOnScrollStop() {
...
//        System.gc();
     ...
     *
     * @param onClickListener | Click the Start button callback function
     */
    @Deprecated
     ...
     *
     * @param thumbScaleType | Thumbnail scaleType
     */
    ...
    class setViewVideo extends AsyncTask<Void, Void, Void> {
        JSONObject jo;
        ...
        protected Void doInBackground(Void... voids) {
            ...
            pair.add(new BasicNameValuePair("video_id", String.valueOf(videoId)));
            return null;
        }
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            postData(video_url, pair, new Handler(new Handler.Callback() {
                @Override
                    ...
                    if (response.getString("error").length() == 0) {
                        ...
                        try {
                            ...
                            if (error == 0) {
                                ...
                                Toast.makeText(getContext(), "Error : Sorry, an error has occurred. Please try again later.", Toast.LENGTH_SHORT).show();
                        } catch (Exception e) {
    ...
    public HttpPost postData(String URL, final List<NameValuePair> params, final Handler handler) {
